...
function Converter() {
    ...
    this.meta.title = "";
    this.meta.date = "";
    this.languageCode = require('./LanguageConst.json');
}

Converter.prototype.run = function (templatePath, filePath, resultPath) {
...
Converter.prototype.generateHTML = function (xml) {
    this.loadStyles();
...
Converter.prototype.addParagraph = function (pText, bodyNode) {
    ...
    if (contentText.length > 0) {
        ...
        if (this.languageProperties) {
            pNode.setAttribute("style", "direction:" + this.languageProperties.direction + ";");
        }
        bodyNode.appendChild(pNode);
    }
    ...
    if (fSize > 0) {
        for (var j = 0; j < fSize; j++) {
            ...
            if (this.languageProperties) {
                pTag.setAttribute("style", "direction:" + this.languageProperties.direction + ";");
            }
            pTag.textContent = val[1];
            aside.appendChild(pTag);
            aside.setAttribute("id", val[0]);
...
Converter.prototype.addFootnote = function (text) {
    for (var key in this.footnoteMap) {
        var val = this.footnoteMap[key];

        var a = "<a data-fnid='" + val[0] + "'></a>";
        text = text.replace(key, a);
    }
    return text;
}

Converter.prototype.loadStyles = function () {
...
Converter.prototype.setMetaData = function () {
    var officeDocumentContent = this.xml.getElementsByTagName("office:document")[0];
...
Converter.prototype.searchBoldAndUnderline = function (span) {
...
Converter.prototype.replaceBoldAndUnderline = function (text) {
